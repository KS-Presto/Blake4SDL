...
// probably go to 3D_MSGS.C and the stats/terminal stuff to 3D_INTER.C
//
...
//#define TERM_BACK_COLOR     2       // defined in 3d_menu.h
...
static unsigned tcursor_y = TERM_SCREEN_YOFS;

const char      TERM_MSG[] = "^ST1^CEEnter commands and press ENTER.\r^CEPress ESC to exit terminal.^XX";
int             TERM_sound_on = 1;
...
#define FreeTerminalCommands() free (TermCommands)
#define FreeTerminalMessages() free (TermMessages)
...
=
= Caches in the commands in TERM_COMMANDS grsegs and parses the file for
...
void LoadTerminalCommands (void)
{
    ...
    {
        //
        // move past any leading returns/linefeeds
        //
        while (*message == '\n' || *message == '\r')
            message++;
        ...
        *message = '\0';
        message += 3;   // move to start of next message
    }
}
...
= CacheTerminalPrint
=
= Prints a message in the TERM_MESSAGES grsegs
=
===============
*/

void CacheTerminalPrint (int msgnum, bool fastprint)
{
    char *message;
    ...
    while (msgnum--)
    {
        ...
        if (!message)
            Quit ("Bad Cached Terminal Message number!");
...
void ActivateTerminal (bool skiplink)
{
    ...
    bool            tempcaps,ExitMoFo;
    unsigned        oldwidth;
    CustomCursor_t  TermCursor = {'@',0,0x58,2};   // Holds Font#, etc.
    int             msgnum;
    ...
    VW_DrawPic (TERM_BACK_XOFS,TERM_BACK_YOFS,TERMINAL_SCREENPIC);
    ...
    while (!ExitMoFo)
    {
        ...
        if (US_LineInput(px + 1,py,buffer,NULL,true,MAX_INPUT,240 + TERM_SCREEN_XOFS - px))
        {
            CacheTerminalPrint (TM_RETURN,false);

            if (*buffer)
            {
                msgnum = US_CheckParm(buffer,Commands);
                ...
                switch (msgnum)
                {
                    ...
                    case TC_ARRIVAL_GOLDSTERN:
                        ...
                        else if (GoldsternInfo.flags == GS_COORDFOUND)
                        {
                            ...
                            snprintf (buffer,sizeof(buffer)," %d^XX",GoldsternInfo.WaitTime / 60);
                            TerminalPrint (buffer,false);
                            CacheTerminalPrint (TM_SECONDS,false);
                        }
                        else if (GoldsternInfo.WaitTime)
                        {
                    ...
                    case TC_PROFILE:
                        ...
                        if (US_LineInput(px + 1,py,buffer,NULL,true,MAX_INPUT,246 + TERM_SCREEN_XOFS - px))
                        {
                            CacheTerminalPrint (TM_RETURN,false);
            ...
            ExitMoFo = true;    // user pressed escape
...
const char *colorcodes[NUMCOLORS] =
{
    "^FC0F",
    "^FC1C",
    "^FC5C",
...
//
int MarkMapPiece (char *work, int pos, int color)
{
    snprintf (&work[pos],WORKBUFFERSIZE - pos,colorcodes[color]);
    pos += sizeof(colorcodes[color]);
...
void FloorCheat (unsigned RadarFlags)
{
    ...
    outbuffer = SafeMalloc(BUFFERSIZE);
    work = SafeMalloc(BUFFERSIZE);
    ...
    for (y = 0; y < mapheight; y++)
    {
        ...
        for (x = 0; x < mapwidth; x++)
        {
            ...
            {
                tile = (uintptr_t)check;

                //
                // 108 == LAST WALL TILE
                //
                if ((tile && tile < 108) || ((tile & 0x80) && !ISPOINTER(tile) && !DebugOk))
                    work[pos++] = '!';    // mark wall
                else
                {
                    //
                    // not a wall
                    //
                    if ((RadarFlags & RS_PERSONNEL_TRACKER) && ISPOINTER(check) && !(check->flags & FL_DEADGUY))
                    {
                        switch (check->obclass)
                        {
                            case playerobj:
                                if (RadarFlags & RS_PERSONNEL_TRACKER)
                                {
                                    ...
                                    //
                                    pos += MarkMapPiece(work,pos,WHITE);
                                    pos += MarkMapPiece(work,pos,NORMAL);
                                }
                                else
                                    work[pos++] = ' ';
                                break;
                            ...
                            case security_lightobj:
                                if (RadarFlags & RS_SECURITY_STATUS)
                                {
                                    ...
                                    //
                                    if (check->temp1)
                                        pos += MarkMapPiece(work,pos,RED1);
                                    else
                                        pos += MarkMapPiece(work,pos,GREEN);
                            ...
                            case goldsternobj:
                                if (RadarFlags & RS_GOLDSTERN_TRACKER)
                                {
                                    ...
                                    //
                                    pos += MarkMapPiece(work,pos,YELLOW);
                                    pos += MarkMapPiece(work,pos,NORMAL);
                                }
                                else
                                    work[pos++] = ' ';
                            ...
                            default:
                                if (RadarFlags & RS_PERSONNEL_TRACKER)
                                {
                                    ...
                                    //
                                    pos += MarkMapPiece(work,pos,RED2);
                                    pos += MarkMapPiece(work,pos,NORMAL);
                                }
                                else
                                    work[pos++] = ' ';
        ...
        // TODO: this may not be correct
        //
        snprintf (outbuffer,BUFFERSIZE,"%s\r\n^XX",work);

        TerminalPrint (outbuffer,true);
    }
